...
class LogLevelSetter:
    ...
    def __exit__(self, a, b, c):
        logger.get(RAFT_NAME).set_level(<int>self.prev_log_level)
...
def set_level(level):
    ...
    level : int
        ...
        It must be one of RAFT.internals.logger.LEVEL_*
    ...
    """
    cdef int prev = logger.get(RAFT_NAME).get_level()
    context_object = LogLevelSetter(prev)
    logger.get(RAFT_NAME).set_level(<int>level)
...
class PatternSetter:
    ...
    def __exit__(self, a, b, c):
        ...
        logger.get(RAFT_NAME).set_pattern(s)
...
def set_pattern(pattern):
    ...
    >>> import RAFT.internals.logger as logger
    >>> logger.set_pattern("--> [%H-%M-%S] %v")
    <RAFT.internals.logger.PatternSetter object at 0x...>
    ...
    """
    cdef string prev = logger.get(RAFT_NAME).get_pattern()
...
def should_log_for(level):
    ...
    level : int
        Logging level to be set. \
        It must be one of RAFT.common.logger.level_*
    """
    return logger.get(RAFT_NAME).should_log_for(<int>level)
...
logger.get(RAFT_NAME).set_callback(_log_callback)
logger.get(RAFT_NAME).set_flush(_log_flush)
