...
cdef extern from "raft_runtime/neighbors/ivf_pq.hpp" \
    ...
    cdef void build(const device_resources& handle,
                    ...
                    const device_matrix_view[float, uint64_t, row_major]& dataset,
                    index[uint64_t]* index) except +

    cdef void build(const device_resources& handle,
                    const index_params& params,
                    const device_matrix_view[int8_t, uint64_t, row_major]& dataset,
                    index[uint64_t]* index) except +
    ...
    cdef void extend(const device_resources& handle,
                     ...
                     const device_matrix_view[float, uint64_t, row_major]& new_vectors,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& new_indices) except +

    cdef void extend(const device_resources& handle,
                     index[uint64_t]* index,
                     const device_matrix_view[int8_t, uint64_t, row_major]& new_vectors,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& new_indices) except +

    cdef void extend(const device_resources& handle,
                     index[uint64_t]* index,
                     const device_matrix_view[uint8_t, uint64_t, row_major]& new_vectors,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& new_indices) except +

    cdef void search(const device_resources& handle,
                     const search_params& params,
                     const index[uint64_t]& index,
                     const device_matrix_view[float, uint64_t, row_major]& queries,
                     uint32_t k,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& neighbors,
                     const device_matrix_view[float, uint64_t, row_major]& distances) except +

    cdef void search(const device_resources& handle,
                     const search_params& params,
                     const index[uint64_t]& index,
                     const device_matrix_view[int8_t, uint64_t, row_major]& queries,
                     uint32_t k,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& neighbors,
                     const device_matrix_view[float, uint64_t, row_major]& distances) except +
    ...
    cdef void search(const device_resources& handle,
                     const search_params& params,
                     const index[uint64_t]& index,
                     const device_matrix_view[uint8_t, uint64_t, row_major]& queries,
                     uint32_t k,
                     const device_matrix_view[uint64_t, uint64_t, row_major]& neighbors,
                     const device_matrix_view[float, uint64_t, row_major]& distances) except +
