...
namespace raft::cluster::kmeans::runtime {
    ...
    void update_centroids(
        ...
        auto min_cluster_and_dist = raft::make_device_vector<raft::KeyValuePair<IndexT, DataT>, IndexT>(handle, n_samples);
        ...
        auto cur_centroids_view = raft::make_device_matrix_view<const DataT, IndexT>(cur_centroids, n_clusters, n_features);
        auto l2norm_x_view = raft::make_device_vector_view<const DataT, IndexT>(l2norm_x, n_samples);
        auto new_centroids_view = raft::make_device_matrix_view<DataT, IndexT>(new_centroids, n_clusters, n_features);
        auto new_weight_view = raft::make_device_vector_view<DataT, IndexT>(new_weight, n_clusters);

        raft::cluster::kmeans::update_centroids<DataT, IndexT>(
                handle,
                X_view,
                weight_view,
                cur_centroids_view,
