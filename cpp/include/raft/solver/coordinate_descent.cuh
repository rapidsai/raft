...
namespace raft::solver::coordinate_descent {
    ...
    void minimize(const raft::handle_t& handle,
        ...
        RAFT_EXPECTS(A.extent(0) == b.extent(0), "Number of labels must match the number of rows in input matrix");

        if(sample_weights.has_value()) {
            RAFT_EXPECTS(A.extent(0) == sample_weights.value().extent(0), "Number of sample weights must match number of rows in input matrix");
        }

        RAFT_EXPECTS(x.extent(0) == A.extent(1), "Objective is linear. The number of coefficients must match the number features in the input matrix");
        RAFT_EXPECTS(lossFunct == loss_funct::SQRD_LOSS, "Only squared loss is supported in the current implementation.");

        math_t *intercept_ptr = intercept.has_value() ? intercept.value().data_handle() : nullptr;
        math_t *sample_weight_ptr = sample_weights.has_value() ? sample_weights.value().data_handle() : nullptr;

        detail::cdFit(handle, A.data_handle(), A.extent(0), A.extent(1),
                      b.data_handle(), x.data_handle(), intercept_ptr,
                      intercept.has_value(), params.normalize, params.epochs,
                      params.loss, params.alpha, params.l1_ratio, params.shuffle,
                      params.tol, sample_weight_ptr);
    }
}