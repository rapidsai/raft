...
using padded_matrix = stdex::mdspan<Type, extents_type, stdex::layout_stride>;
                               ...
                               Vecs... vecs)
{
  typedef raft::Pow2<VecBytes> AlignBytes;
  constexpr std::size_t VecElems     = VecBytes / sizeof(Type);
  constexpr std::size_t PaddingElems = PaddingBytes / sizeof(Type);
  typedef raft::Pow2<PaddingElems> AlignPadding;

  const uint paddedRowLen  = AlignPadding::roundUp(rowLen);
  const IdxType alignedLen = paddedRowLen * nRows;

  // some runtime checks for now -- should be statically enforced
  assert(in.data() == AlignBytes::roundUp(in.data()));
  assert(in.size() == alignedLen);
  assert(in.is_strided());
  assert(in.stride(0) == paddedRowLen);
...
// FIXME: How to statically ensure that 'padded_matrix' is using 'PaddingBytes'?
template <std::size_t VecBytes = 16, std::size_t PaddingBytes = 128, int BlockSize = 256>
