...
void rbc_all_knn_query(const raft::handle_t &handle,
  ...
  thrust::sequence(thrust::cuda::par.on(handle.get_stream()), index.get_R_1nn_cols(),
                   index.get_R_1nn_cols() + index.m, (value_idx)0);


  // TODO: Separate the sampling logic into its own function
  ...
  rng.sampleWithoutReplacement(
    handle, R_indices.data(), R_1nn_cols2.data(), index.get_R_1nn_cols(),
    R_1nn_ones.data(), (value_idx)index.n_landmarks, (value_idx)index.m, handle.get_stream());
  ...
  brute_force_knn_impl<int, int64_t>(
    input, sizes, index.n, const_cast<value_t *>(index.get_X()), index.m, R_knn_inds.data(),
    R_knn_dists.data(), k, handle.get_device_allocator(), handle.get_stream(),
    nullptr, 0, (bool)true, (bool)true, nullptr, index.metric);
   ...
   */
   // TODO: Separate 1nn logic into its own function
  ...
  thrust::for_each(exec_policy, idxs, idxs + index.m, [=] __device__(value_idx i) {
  ...
  auto vals = thrust::make_zip_iterator(thrust::make_tuple(index.get_R_1nn_cols()));
   ...
   * (need to take the
   */
  value_idx *R_indptr_ptr = index.get_R_indptr();
  ...
  // TODO: Separate these calls into their own files / functions
  ...
  if (index.n <= 2) {
    ...
    block_rbc_kernel_registers<value_idx, value_t, 32, 2, 128, 2>
      <<<index.m, 128, 0, handle.get_stream()>>>(
        index.get_X(), index.n, R_knn_inds.data(), R_knn_dists.data(), index.m, k, index.get_R_indptr(),
        index.get_R_1nn_cols(), index.get_R_1nn_dists(), inds, dists, R_indices.data(),
  ...
  if (perform_post_filtering) {
    ...
    if (index.n <= 2) {
      ...
      <<<index.m, 128, bitset_size * sizeof(uint32_t), handle.get_stream()>>>(
        index.get_X(), index.n, R_knn_inds.data(), R_knn_dists.data(), index.get_R_radius(), index.get_R(),
        index.n_landmarks, bitset_size, k, dfunc, bitset.data(), weight);
      ...
      compute_final_dists_registers<<<index.m, 128, 0, handle.get_stream()>>>(
        index.get_X(), index.n, bitset.data(), bitset_size, R_knn_dists.data(), index.get_R_indptr(),
        index.get_R_1nn_cols(), index.get_R_1nn_dists(), inds, dists, index.n_landmarks, k, dfunc,
      ...
      <<<index.m, 128, bitset_size * sizeof(uint32_t), handle.get_stream()>>>(
        index.get_X(), index.n, R_knn_inds.data(), R_knn_dists.data(), index.get_R_radius(), index.get_R(),
        index.n_landmarks, bitset_size, k, bitset.data(),
        raft::sparse::distance::SqDiff(), raft::sparse::distance::Sum(), weight);
