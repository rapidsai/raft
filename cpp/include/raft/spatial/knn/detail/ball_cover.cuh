...
__global__ void rbc_kernel(const value_t *X, const value_idx n_cols,
  ...
  heap.reduce();
  heap.writeOut(out_dists + (row * k), out_inds + (row * k), k);
}
...
void random_ball_cover(const raft::handle_t &handle, const value_t *X,
  ...
  //  I think this might be assuming col-major?
  //  raft::matrix::sliceMatrix(R_knn_inds.data(), m, k, R_1nn_inds.data(), 0, 0,
  //                            m+1, 1, handle.get_stream());
  //  raft::matrix::sliceMatrix(
  //    R_knn_dists.data(), m, k, R_1nn_dists.data(), 0,
  //    0, m+1, 1, handle.get_stream());
  //
  ...
  rbc_kernel<<<m, 32, 0, handle.get_stream()>>>(
