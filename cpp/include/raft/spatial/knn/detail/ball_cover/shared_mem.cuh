...
template <typename value_idx,
          ...
          int numthreadq,
          int tpb,
          typename reduce_func,
          typename accum_func>
__device__ int compute_dist_full_warp(
  faiss::gpu::KeyValueBlockSelect<value_t, int, Dir, Comp, numwarpq, numthreadq, tpb>& heap,
  const int lane_id,
  const int n_cols,
  ...
  accum_func accfunc)
{
  unsigned int mask = __ballot_sync(0xffffffff, z <= heap.warpKTop && cur_candidate_ind > -1);
  ...
  while (__popc(mask) > 0) {
    ...
    if (lane_id == lowest_peer) ++n_dists;
  ...
  accum_func accfunc)
{
  ...
  for (; i < limit; i += tpb) {
     ...
     *d(l_2, t) | - d(s, l_1) * d(l_2, t) Therefore, if d(s, t) >= d(s, l_1) from the computation
     *above, we know that the distance to the candidate point cannot possibly be in the nearest
     *neighbors. However, if d(s, t) < d(s, l_1) then we should compute the distance because it's
     *possible it could be smaller.
     **/
    value_t z =
      heap.warpKTopRDist == 0.00
        ? 0.0
        : (abs(heap.warpKTop - heap.warpKTopRDist) * abs(heap.warpKTopRDist - cur_candidate_dist) -
           heap.warpKTop * cur_candidate_dist) /
            heap.warpKTopRDist;
    z = isnan(z) ? 0.0 : z;

    n_dists += compute_dist_full_warp<value_idx,
                                      value_t,
                                      false,
  ...
  if (warp_id == (R_size / faiss::gpu::kWarpSize) % kNumWarps) {
    const value_idx cur_candidate_ind = i < R_size ? R_1nn_cols[R_start_offset + i] : -1;
    const value_t cur_candidate_dist =
      i < R_size ? R_1nn_dists[R_start_offset + i] : std::numeric_limits<value_t>::max();
...
template <typename value_idx,
          ...
          typename bitset_type = uint32_t,
          typename dist_func,
          typename reduce_func,
          typename accum_func>
__global__ void compute_final_dists_smem(const value_t* __restrict__ X,
                                         ...
                                         value_int* dist_counter)
{
  ...
  __shared__ faiss::gpu::KeyValuePair<value_t, int> smemV[kNumWarps * warp_q];
  ...
  faiss::gpu::
    KeyValueBlockSelect<value_t, int, false, faiss::gpu::Comparator<value_t>, warp_q, thread_q, tpb>
  ...
  for (int cur_R_ind = 0; cur_R_ind < n_landmarks; cur_R_ind++) {
    ...
    if (_get_val(bitset + (blockIdx.x * bitset_size), cur_R_ind)) {
      ...
      n_dists += compute_dist_block<value_idx,
                                    value_t,
                                    false,
                                    faiss::gpu::Comparator<value_t>,
                                    warp_q,
                                    ...
                                    float weight = 1.0)
{
  ...
  for (int l = warp_id; l < n_landmarks; l += kNumWarps) {
    ...
    if (compute) {
      ...
      if (lane_id == 0 && in_bound) { _zero_bit(smem, l); }
    }
  }
...
template <typename value_idx = int64_t,
          ...
          typename reduce_func,
          typename accum_func>
__global__ void block_rbc_kernel_smem(const value_t* __restrict__ X,
                                      ...
                                      float weight = 1.0)
{
  ...
  __shared__ value_t smemK[kNumWarps * warp_q];
  __shared__ faiss::gpu::KeyValuePair<value_t, int> smemV[kNumWarps * warp_q];
  __shared__ value_t shared_x_ptr[col_q];  // TODO: cols_q should be power of 2 up to some max val
