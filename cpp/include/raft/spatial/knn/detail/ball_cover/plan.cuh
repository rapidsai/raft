 ...
 */
template <typename value_t, typename value_idx, typename value_int = std::uint32_t>
__device__ bool should_prune(value_idx l,
                             value_int k,
                             value_t p_q_r,
                             value_t closest_R_dist,
                             const value_t* R_radius,
                             const value_t knn_dist,
                             float weight)
{
  return p_q_r > weight * (closest_R_dist + R_radius[l]) || p_q_r > 3 * closest_R_dist ||
         knn_dist < p_q_r - R_radius[l];
}
...
struct TupleComp {
  ...
  {
    // sort first by query id
    if (thrust::get<0>(t1) < thrust::get<0>(t2)) return true;
    if (thrust::get<0>(t1) > thrust::get<0>(t2)) return false;

    // then sort by landmark distance (ascending order)
...
void order_plan_incremental(const raft::handle_t& handle,
                            value_idx* plan_csr,
                            ...
                            value_int n_query_pts)
{
  ...
  auto initial_keys =
    thrust::make_zip_iterator(thrust::make_tuple(plan_coo.rows(), batch_landmark_dists));
  auto initial_vals = thrust::make_zip_iterator(
    thrust::make_tuple(plan_coo.rows(), plan_coo.cols(), plan_coo.vals()));
  thrust::sort_by_key(handle.get_thrust_policy(),
                      initial_keys,
                      initial_keys + plan_coo.nnz,
                      initial_vals,
                      TupleComp());

  CUDA_CHECK(cudaStreamSynchronize(handle.get_stream()));
  std::cout << "Initial sort by key" << std::endl;

  // For each query id, increment index for each partition by n_query_pts. It would be nice if there
  // were a way to affectively transpose the matrix into column-major ordering without losing the
  // ordering of the partitions wrt query ids but for now we create an index to do that ordering (by
  // taking a cumulative sum over the leading dimension)
  rmm::device_uvector<value_idx> sort_idx(plan_coo.nnz, handle.get_stream());
  build_part_sort_index<<<raft::ceildiv((std::uint64_t)n_query_pts, (std::uint64_t)256),
                          256,
                          0,
                          handle.get_stream()>>>(plan_csr, sort_idx.data(), n_query_pts);
  ...
  auto final_vals = thrust::make_zip_iterator(
    thrust::make_tuple(plan_coo.rows(), plan_coo.cols(), plan_coo.vals()));
  thrust::sort_by_key(
    handle.get_thrust_policy(), sort_idx.data(), sort_idx.data() + plan_coo.nnz, final_vals);
...
__global__ void prune_landmarks(const value_t* landmark_dists,
...
#pragma unroll
  for (int i = threadIdx.x; i < bitset_size; i += tpb)
    sh_mem[i] = 0xffffffff;
...
#pragma unroll
  for (int l = threadIdx.x; l < n_landmarks; l += tpb) {
    ...
    if (should_prune(
          l, k, p_q_r, closest_R_dist, R_radius, knn_dists[blockIdx.x * k + (k - 1)], weight)) {
      _zero_bit(sh_mem, l);
    } else {
                           ...
                           value_t* knn_dists)
{
  ...
  if (i < k) { heap.addThreadQ(knn_dists[query_id * k + i], knn_inds[query_id * k + i]); }
  heap.checkThreadQ();
  heap.reduce();
                              ...
                              float weight)
{
  ...
  if (should_prune(landmark_id,
                   k,
                   landmark_dists[blockIdx.x],
                   R_knn_dists[query_id * k + (k - 1)],
                   R_radius,
                   knn_dists[query_id * k + (k - 1)],
                   weight)) {
    return;
  }
  ...
  int working_batch_size = min(R_indptr[landmark_id + 1] - offset_start, (value_idx)batch_size);
  ...
  // Can break this up to chunk d  >= 16 && d < 32 -> half-warp,
  ...
  for (int i = 0; i < working_batch_size; ++i) {
    ...
    if (lane_id == 0) { point_index = R_1nn_cols[offset_start + i]; }
...
template <typename value_idx,
          ...
          typename value_int = std::uint32_t,
          int batch_size     = 64>
void compute_and_execute_plan(const raft::handle_t& handle,
                              BallCoverIndex<value_idx, value_t, value_int>& index,
                              value_int k,
                              const value_t* query,
                              const value_int n_query_pts,
                              ...
                              float weight = 1.0)
{
  ...
  k_closest_landmarks2(handle, index, query, n_query_pts, k, R_knn_inds.data(), R_knn_dists.data());
  ...
  thrust::fill(
    handle.get_thrust_policy(), landmark_batches.data(), landmark_batches.data() + n_query_pts, 0);
  ...
  prune_landmarks<value_idx, value_t, value_int, 128>
    <<<n_query_pts, 128, bitset_size * sizeof(value_int), handle.get_stream()>>>(
      ...
      batch_size,
      bitset.data(),
      landmark_batches.data(),
      weight);

  // Sum of cardinality array is nnz of plan
  value_idx n_batches = thrust::reduce(
    handle.get_thrust_policy(), landmark_batches.data(), landmark_batches.data() + n_query_pts, 0);

  raft::print_device_vector("landmark_batches", landmark_batches.data(), 25, std::cout);

  rmm::device_uvector<value_idx> coo_write_plan(n_query_pts + 1, handle.get_stream());
  ...
  raft::update_device(coo_write_plan.data() + n_query_pts, &n_batches, 1, handle.get_stream());

  // Construct COO where nnz=n_batches
  plan_coo.allocate(n_batches, 0, handle.get_stream());

  rmm::device_uvector<value_t> batch_landmark_dists(n_batches, handle.get_stream());

  write_plan_coo<<<raft::ceildiv(n_query_pts, (value_int)256), 256, 0, handle.get_stream()>>>(
    index.get_R_indptr(),
    coo_write_plan.data(),
    bitset.data(),
    bitset_size,
  ...
  order_plan_incremental(
    handle, coo_write_plan.data(), plan_coo, batch_landmark_dists.data(), n_query_pts);
  ...
  // TODO: Batch through this
  rmm::device_uvector<int> mutex(n_query_pts, handle.get_stream());
  thrust::fill(handle.get_thrust_policy(), mutex.data(), mutex.data() + n_query_pts, 0);

  CUDA_CHECK(cudaStreamSynchronize(handle.get_stream()));

  compute_dists<value_idx, value_t, value_int, 32, 2, 128, batch_size>
    <<<plan_coo.nnz, 128, 0, handle.get_stream()>>>(index.get_X(),
                                                    query,
                                                    k,
                                                    index.n,
                                                    index.get_R_indptr(),
  ...
  CUDA_CHECK(cudaStreamSynchronize(handle.get_stream()));
  std::cout << "Compute dists time: " << (curTimeMillis() - compute_dists_time) / 1000.0
            << std::endl;
}
