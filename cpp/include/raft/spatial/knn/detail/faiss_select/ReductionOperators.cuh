...
#include <cuda.h>
#include <raft/spatial/knn/detail/faiss_select/Limits.cuh>
#include <raft/spatial/knn/detail/faiss_select/MathOperators.cuh>
#include <raft/spatial/knn/detail/faiss_select/Pair.cuh>

namespace raft::spatial::knn::detail::faiss_select {

template <typename T>
struct Sum {
  __device__ inline T operator()(T a, T b) const { return Math<T>::add(a, b); }

  inline __device__ T identity() const { return Math<T>::zero(); }
};
...
template <typename T>
struct Max {
  __device__ inline T operator()(T a, T b) const { return Math<T>::gt(a, b) ? a : b; }
...
struct SegmentedReduce {
  ...
  __device__ inline Pair<T, bool> operator()(const Pair<T, bool>& a, const Pair<T, bool>& b) const
  {
    return Pair<T, bool>(b.v ? b.k : op(a.k, b.k), a.v || b.v);
  }

  inline __device__ Pair<T, bool> identity() const { return Pair<T, bool>(op.identity(), false); }
