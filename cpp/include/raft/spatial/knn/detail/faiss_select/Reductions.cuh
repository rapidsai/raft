...
__device__ inline T blockReduceAll(T val, Op op, T* smem)
{
  ...
  if (warpId == 0) {
    val = laneId < utils::divUp(blockDim.x, kWarpSize) ? smem[laneId] : op.identity();
    val = warpReduceAll<T, Op>(val, op);

    if (BroadcastAll) {
      __threadfence_block();

      if (laneId == 0) { smem[0] = val; }
    }
  }
  ...
  if (KillWARDependency) { __syncthreads(); }
...
#pragma unroll
    for (int i = 0; i < Num; ++i) {
      val[i] =
        laneId < utils::divUp(blockDim.x, kWarpSize) ? smem[laneId * Num + i] : op.identity();
      val[i] = warpReduceAll<T, Op>(val[i], op);
    }
...
{
  return blockReduceAll<T, Sum<T>, BroadcastAll, KillWARDependency>(val, Sum<T>(), smem);
}
...
{
  return blockReduceAll<Num, T, Sum<T>, BroadcastAll, KillWARDependency>(vals, Sum<T>(), smem);
}
