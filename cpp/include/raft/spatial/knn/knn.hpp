 ...
 *            as input vector.
 * @tparam idx_t The type of returned indices (either int or int64_t)
 * @tparam value_t The type of input and search_items
 * @tparam int_t The type for dimensions.
 */
...
{
  // remove_reference makes translations's type appear in a non-deduced context,
  // allowing the template parameters to be deduced when translations==nullptr.
