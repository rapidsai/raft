...
class cusolver_sp_resource_t : public resource_t {
 public:
  cusolver_sp_resource_t(cudaStream_t stream)
  {
  ...
  ~cusolver_sp_resource_t() { RAFT_CUSOLVER_TRY_NO_THROW(cusolverSpDestroy(cusolver_handle)); }
...
class cusolver_sp_resource_factory_t : public resource_factory_t {
  resource_type_t resource_type() { return resource_type_t::CUSOLVER_SP_HANDLE; }
  resource_t* make_resource() { return new cusolver_sp_resource_t(); }
};
 ...
 * @return
 */
cusolvereHandle_t get_cusolver_sp_handle(raft::base_handle const& handle)
{
  ...
  return *handle.get_resource<cusolverHandle_t>(resource_type_t::CUSOLVER_SP_HANDLE);
};
