...
class cuda_event_resource_t : public resource_t {
 public:
  ...
  ~cuda_event_resource_t() override { RAFT_CUDA_TRY_NO_THROW(cudaEventDestroy(event_)); }
...
class cuda_stream_sync_event_resource_factory_t : public resource_factory_t {
 public:
  resource_type_t resource_type() override { return resource_type_t::CUDA_STREAM_SYNC_EVENT; }
  resource_t* make_resource() override { return new cuda_event_resource_t(); }
};
