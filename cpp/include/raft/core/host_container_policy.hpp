...
struct pinned_vector {
 ...
 public:
  ...
  pinned_vector(pinned_vector const& that) : data_{that.data_} {}

  auto operator=(pinned_vector<T> const& that) -> pinned_vector<T>&
  {
    data_ = underlying_container_type{that.data_};
    return *this;
  }
  auto operator=(pinned_vector<T>&& that) noexcept -> pinned_vector<T>& = default;
...
struct pinned_vector_policy {
  using element_type          = ElementType;
  using container_type        = pinned_vector<element_type>;
  using allocator_type        = typename container_type::allocator_type;
  using pointer               = typename container_type::pointer;
  using const_pointer         = typename container_type::const_pointer;
