...
size_t csr_add_calc_inds(const int *a_ind, const int *a_indptr, const T *a_val,
                         ...
                         cudaStream_t stream) {
  return detail::csr_add_calc_inds(a_ind, a_indptr, a_val, nnz1, b_ind,
                                   b_indptr, b_val, nnz2, m, out_ind, stream);
}
...
void csr_add_finalize(const int *a_ind, const int *a_indptr, const T *a_val,
  ...
  detail::csr_add_finalize(a_ind, a_indptr, a_val, nnz1, b_ind, b_indptr, b_val,
                           nnz2, m, c_ind, c_indptr, c_val, stream);
}
