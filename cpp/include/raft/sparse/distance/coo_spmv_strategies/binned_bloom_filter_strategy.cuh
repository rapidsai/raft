...
class binned_bloom_filter_strategy
  : public coo_spmv_strategy<value_idx, value_t> {
 public:
  ...
  void block_divider(
    ...
    std::tuple<value_idx, value_idx, value_idx, value_idx> &block_counts) {
    ...
    auto mask_128 =
      thrust::copy_if(policy, thrust::make_counting_iterator(0),
                      thrust::make_counting_iterator(n_rows),
                      mask_indptr.data(), nnz_filter_functor<128>(indptr));
    std::get<0>(block_counts) = mask_128 - mask_indptr.data();
    // std::cout << "128 Block Rows: " << std::get<0>(block_counts) << std::endl;

    auto mask_256 = thrust::copy_if(policy, thrust::make_counting_iterator(0),
                                    thrust::make_counting_iterator(n_rows),
                                    mask_128, nnz_filter_functor<256>(indptr));
    std::get<1>(block_counts) = mask_256 - mask_128;
    // std::cout << "256 Block Rows: " << std::get<1>(block_counts) << std::endl;

    auto mask_512 = thrust::copy_if(policy, thrust::make_counting_iterator(0),
                                    thrust::make_counting_iterator(n_rows),
                                    mask_256, nnz_filter_functor<512>(indptr));
    std::get<2>(block_counts) = mask_512 - mask_256;
    // std::cout << "512 Block Rows: " << std::get<2>(block_counts) << std::endl;

    auto mask_1024 =
      thrust::copy_if(policy, thrust::make_counting_iterator(0),
                      thrust::make_counting_iterator(n_rows), mask_512,
                      nnz_filter_functor<1024>(indptr));
  ...
  void dispatch(value_t *out_dists, value_idx *coo_rows_b,
    ...
    rmm::device_uvector<value_idx> mask_indptr(this->config.a_nrows,
                                               this->config.stream);
    auto block_counts =
      std::make_tuple(value_idx(0), value_idx(0), value_idx(0), value_idx(0));
    block_divider(this->config.a_indptr, this->config.a_nrows, mask_indptr,
                  block_counts);
    ...
    if (std::get<0>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.b_nnz, chunk_size * 128);

      mask_row_it<value_idx> row_it_128(
        this->config.a_indptr, std::get<0>(block_counts), mask_indptr.data());
      this->_dispatch_base(
        *this, filter_bits<128>(), nnz_in_filter<128>(), row_it_128, out_dists,
        coo_rows_b, product_func, accum_func, write_func, chunk_size,
        row_it_128.n_rows * n_blocks_per_row, n_blocks_per_row, 128);
    }

    if (std::get<1>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.b_nnz, chunk_size * 256);

      mask_row_it<value_idx> row_it_256(
        this->config.a_indptr, std::get<1>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts));
      this->_dispatch_base(
        *this, filter_bits<256>(), nnz_in_filter<256>(), row_it_256, out_dists,
        coo_rows_b, product_func, accum_func, write_func, chunk_size,
    ...
    if (std::get<2>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.b_nnz, chunk_size * 512);

      mask_row_it<value_idx> row_it_512(
        this->config.a_indptr, std::get<2>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts) +
          std::get<1>(block_counts));
      this->_dispatch_base(
        *this, filter_bits<512>(), nnz_in_filter<512>(), row_it_512, out_dists,
        coo_rows_b, product_func, accum_func, write_func, chunk_size,
    ...
    if (std::get<3>(block_counts) > 0) {
      ...
      mask_row_it<value_idx> row_it_1024(
        this->config.a_indptr, std::get<3>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts) +
          std::get<1>(block_counts) + std::get<2>(block_counts));
      this->_dispatch_base(
        *this, filter_bits<1024>(), nnz_in_filter<1024>(), row_it_1024,
        out_dists, coo_rows_b, product_func, accum_func, write_func, chunk_size,
  ...
  void dispatch_rev(value_t *out_dists, value_idx *coo_rows_b,
    ...
    auto block_counts =
      std::make_tuple(value_idx(0), value_idx(0), value_idx(0), value_idx(0));
    block_divider(this->config.b_indptr, this->config.b_nrows, mask_indptr,
                  block_counts);
    ...
    if (std::get<0>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.a_nnz, chunk_size * 128);

      mask_row_it<value_idx> row_it_128(
        this->config.b_indptr, std::get<0>(block_counts), mask_indptr.data());
      this->_dispatch_base_rev(
        *this, filter_bits<128>(), nnz_in_filter<128>(), row_it_128, out_dists,
        coo_rows_b, product_func, accum_func, write_func, chunk_size,
    ...
    if (std::get<1>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.a_nnz, chunk_size * 256);

      mask_row_it<value_idx> row_it_256(
        this->config.b_indptr, std::get<1>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts));
      this->_dispatch_base_rev(
        *this, filter_bits<256>(), nnz_in_filter<256>(), row_it_256, out_dists,
        coo_rows_b, product_func, accum_func, write_func, chunk_size,
    ...
    if (std::get<2>(block_counts) > 0) {
      auto n_blocks_per_row =
        raft::ceildiv(this->config.a_nnz, chunk_size * 512);

      mask_row_it<value_idx> row_it_512(
        this->config.b_indptr, std::get<2>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts) +
          std::get<1>(block_counts));
      this->_dispatch_base_rev(
        *this, filter_bits<512>(), nnz_in_filter<512>(), row_it_512, out_dists,
    ...
    if (std::get<3>(block_counts) > 0) {
      ...
      mask_row_it<value_idx> row_it_1024(
        this->config.b_indptr, std::get<3>(block_counts),
        mask_indptr.data() + std::get<0>(block_counts) +
          std::get<1>(block_counts) + std::get<2>(block_counts));
      this->_dispatch_base_rev(
        *this, filter_bits<1024>(), nnz_in_filter<1024>(), row_it_1024,
