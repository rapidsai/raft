...
class csparse_device_view_t {
 public:
  ...
  csparse_device_view_t(idx_t nvec,  // total number of row vectors for csr or col vectors for csc
    ...
    idx_t major_idx) const noexcept
  {
    ...
    auto values  = values_ ? thrust::optional<value_t const*>{*values_ + offset} : thrust::nullopt;
...
class coo_device_view_t {
 public:
  // TODO: Use a strategy / policy to separate host from device views
  coo_device_view_t(src_idx_t nvec,  // total number of source vectors
