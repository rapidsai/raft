...
// /**
//  * Unpack 1 record of a single list (cluster) in the index to fetch the flat (compressed) PQ
//  code.
...
// /**
//  * Write one flat (compressed) PQ code into a block by the given offset. The offset indicates the
//  id
//  * of the record in the list. This function interleaves the code and writes the interleaved
//  result
//  * to the output. NB: no memory allocation happens here; the block must fit the record (offset +
//  1).
//  *
//  * @param[in] block interleaved block. Pointer to the inverted list data in interleaved format
//  * @param[out] flat_code output flat PQ code
...
//  */
// template <uint32_t PqBits>
// void pack_pq(raft::resources handle, const uint8_t* flat_codes, uint8_t* block, uint32_t pq_dim,
// uint32_t num_codes, uint32_t offset, uint32_t batch_size=0)
// {
//   auto spec = list_spec(PqBits, pq_dim, false);
//   auto extents = spec.make_list_extents(num_codes);
//   auto list_view = make_readonly_temporary_device_buffer<const uint8_t>(handle, flat_codes,
//   extents).view(); ivf_pq::detail::pack_list_data(block, offset, 0, pq_dim,
//   ivf_pq::detail::pass_codes{flat_code});
// }

// inline void unpack(
//   raft::resources const& res, uint8_t* list_data, uint32_t pq_bits, uint32_t offset, uint8_t*
//   codes)
// {
//   auto extents = raft::make_extents<>();
//   auto make_writeback_temporary_device_buffer(
//     raft::resources const& handle, ElementType* data, raft::extents<IndexType, Extents...>
//     extents) make_writeback_temporary_device_buffer<typename ElementType>(res, codes, )
...
// {
//   ivf_pq::detail::pack_list_data(list_data, codes, offset, pq_bits,
