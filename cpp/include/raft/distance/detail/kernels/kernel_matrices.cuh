...
RAFT_KERNEL_ void polynomial_kernel_nopad(
 ...
 */
template <typename math_t, typename exp_t>
RAFT_KERNEL_ void polynomial_kernel(
...
RAFT_KERNEL_ void tanh_kernel_nopad(math_t* inout, size_t len, math_t gain, math_t offset)
{
  for (size_t tid = threadIdx.x + blockIdx.x * blockDim.x; tid < len;
       tid += blockDim.x * gridDim.x) {
 ...
 */
template <typename math_t>
RAFT_KERNEL_ void tanh_kernel(math_t* inout, int ld, int rows, int cols, math_t gain, math_t offset)
{
  for (size_t tidy = threadIdx.y + blockIdx.y * blockDim.y; tidy < cols;
       tidy += blockDim.y * gridDim.y)
    for (size_t tidx = threadIdx.x + blockIdx.x * blockDim.x; tidx < rows;
 ...
 */
template <typename math_t>
RAFT_KERNEL_ void rbf_kernel_expanded(
