...
struct PairwiseDistanceGemm {
  ...
  using ThreadblockShape = cutlass::gemm::GemmShape<128, 128, 16>;  // <- threadblock tile M = 128, N = 64, K = 16
  ...
  using GemmBase = typename DefaultGemmUniversal<
    ElementA_, LayoutA_, cutlass::ComplexTransform::kNone, kAlignmentA,
    ElementB_, LayoutB_, cutlass::ComplexTransform::kNone, kAlignmentB,
    ElementC_, LayoutOutput, ElementAccumulator,
    OperatorClass,
    ArchTag,
    ThreadblockShape,
    WarpShape,
...
template <
  ...
  bool isRowMajor
>
struct PairwiseDistanceGemm<double, kAlignmentA, double, kAlignmentB, ElementC_, ElementAccumulator,
                            EpilogueOutputOp, Stages, isRowMajor> {

  //using Transform = cutlass::ComplexTransform::kNone;
  // Threadblock-level tile size (concept: GemmShape)
  using ThreadblockShape = cutlass::gemm::GemmShape<64, 64, 16>;  // <- threadblock tile M = 128, N = 64, K = 16
  /// Warp-level tile size (concept: GemmShape)
  // This code section describes tile size a warp will compute
  ...
  using GemmBase = typename DefaultGemmUniversal<
    double, LayoutA_, cutlass::ComplexTransform::kNone, 1,
    double, LayoutB_, cutlass::ComplexTransform::kNone, 1,
    ElementC_, LayoutOutput, ElementAccumulator,
    OperatorClass,
    ArchTag,
    ThreadblockShape,
