...
public:
  ...
  struct Params {
    FinalOp_ final_op_tmp;
    DistanceOp_ dist_op_tmp;

    //
    // Methods
    //
    CUTLASS_HOST_DEVICE
    Params(DistanceOp_ dist_op, FinalOp lambdafn_): 
      final_op_tmp(lambdafn_),
      dist_op_tmp(dist_op)
      { 
      }
...
public:
  ...
  PairwiseDistanceEpilogueElementwise(Params const &params) : 
    final_op(params.final_op_tmp), elementwise_op(params.dist_op_tmp)  {
  }
