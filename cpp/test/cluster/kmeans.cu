 ...
 class KmeansTest : public ::testing::TestWithParam<KmeansInputs<T>> {
  protected:
   ...
   void basicTest()
   {
     ...
     auto d_centroids_opt = std::make_optional(raft::make_device_matrix_view<T>(d_centroids.data(), params.n_clusters, n_features));
     if (testparams.weighted) {
       d_sample_weight.resize(n_samples, stream);
       d_sw = std::make_optional(raft::make_device_vector_view<T>(d_sample_weight.data(), n_samples));
       thrust::fill(
     ...
     raft::cluster::kmeans_fit_predict<T, int>(handle,
                         params,
                         X.view(),
                         d_sw,
                         d_centroids_opt,
     ...
     score = raft::stats::adjusted_rand_index(d_labels_ref.data(), d_labels.data(), n_samples, handle.get_stream());
