...
TODO : Once fusedL2NN is specialized in the raft_distance shared library,
       add the following :
  ...
  namespace raft
{
  ...
  class FusedL2NNTest : public ::testing::TestWithParam<Inputs<DataT>> {
   ...
   protected:
    ...
    virtual void generateGoldenResult()
    {
      ...
      int k = params.k;
      naive<DataT, Sqrt>(
        min_ref.data(), x.data(), y.data(), m, n, k, (int*)workspace.data(), stream);
    }
  ...
  TEST_P(FusedL2NNDetTestD_Sq, Result)
  {
    ...
    for (int i = 0; i < NumRepeats; ++i) {
      runTest(min1.data());
      ASSERT_TRUE(
        devArrMatch(min.data(), min1.data(), params.m, CompareExactKVP<double>(), stream));
    }
  }
