...
__global__ void decompress_bits_kernel(const T* in, int in_rows, int in_cols, bool* out)
{
  ...
  const size_t out_cols = in_rows * bits_per_element;
  const size_t out_i    = j;
  const size_t out_j    = i * bits_per_element;

  for (int bitpos = 0; bitpos < bits_per_element; ++bitpos) {
    bool bit = ((T(1) << bitpos) & bitfield) != 0;
    if (out_i < out_rows && out_j < out_cols) {
      out[out_i * out_cols + out_j + bitpos] = bit;
    }
  }
}
 ...
 */
template <typename T = uint64_t, typename = std::enable_if_t<std::is_integral<T>::value>>
void decompress_bits(
  const raft::handle_t& handle, const T* in, int in_rows, int in_cols, bool* out)
{
...
{
  return raft::util::itertools::product<Params>(
    {1, 3, 32, 33, 63, 64, 65, 10013},
    {1, 3, 32, 33, 63, 64, 65, 13001});
}
...
void check_invertible(const Params& p) {
  ...
  int m = p.m;
  int n = raft::round_up_safe(p.n, bits_per_elem);
  ...
  int tmp_m = raft::ceildiv(n, bits_per_elem);
  int tmp_n = m;

  int out_m = tmp_n;
  int out_n = tmp_m * bits_per_elem;

  auto tmp = raft::make_device_matrix<T, int>(handle, tmp_m, tmp_n);
  auto out = raft::make_device_matrix<bool, int>(handle, out_m, out_n);

  ASSERT_EQ(in.extent(0), out.extent(0)) << "M does not match";
  ASSERT_EQ(in.extent(1), out.extent(1)) << "N does not match";

  compress_to_bits(handle, in.data_handle(), in.extent(0), in.extent(1), tmp.data_handle());
...
INSTANTIATE_TEST_CASE_P(CompressToBits, CompressToBitsTest, ::testing::ValuesIn(gen_params()));
