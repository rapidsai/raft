...
class SparsePreprocessCoo
 ...
 public:
  ...
  // bm25_vals(params.bm25_vals_h.size(), stream)
  // tfidf_vals(params.tfidf_vals_h.size(), stream),
  // out_idxs_bm25(params.out_idxs_bm25_h.size(), stream),
  // out_idxs_tfidf(params.out_idxs_tfidf_h.size(), stream),
  // out_dists_bm25(params.out_dists_bm25_h.size(), stream),
  // out_dists_tfidf(params.out_dists_tfidf_h.size(), stream)
  {
  }
 ...
 protected:
  ...
  void Run(bool bm25_on)
  {
    ...
    std::vector<Type_f> out_dists_bm25_h = {0, 0.281047, 0, 0,        0, 0,        0, 0.281047,
    ...
    auto bm25_vals      = raft::make_device_vector<float, int64_t>(handle, bm25_vals_h.size());
    auto tfidf_vals     = raft::make_device_vector<float, int64_t>(handle, tfidf_vals_h.size());
    auto out_idxs_bm25  = raft::make_device_vector<int, int64_t>(handle, out_idxs_bm25_h.size());
    // auto out_idxs_tfidf  = raft::make_device_vector<int, int64_t>(handle,
    // params.out_idxs_tfidf_h.size());
    auto out_dists_bm25 = raft::make_device_vector<float, int64_t>(handle, out_dists_bm25_h.size());
    // auto out_dists_tfidf = raft::make_device_vector<float, int64_t>(handle,
    // params.out_dists_tfidf_h.size());
    ...
    // raft::copy(out_idxs_tfidf.data_handle(), params.out_idxs_tfidf_h.data(),
    // params.out_idxs_tfidf_h.size(), stream);
    raft::copy(
      out_dists_bm25.data_handle(), out_dists_bm25_h.data(), out_dists_bm25_h.size(), stream);
    // raft::copy(out_dists_tfidf.data_handle(), params.out_dists_tfidf_h.data(),
    // params.out_dists_tfidf_h.size(), stream);
    ...
    auto c_matrix = raft::make_device_coo_matrix<float, int, int, int>(handle, coo_struct_view);
    ...
    if (bm25_on) {
      ...
      ASSERT_TRUE(raft::devArrMatch<Type_f>(bm25_vals.data_handle(),
                                            result.data_handle(),
                                            result.size(),
                                            raft::CompareApprox<Type_f>(1e-6),
                                            stream));
    } else {
    ...
    //     out_idxs_tfidf.data_handle(), out_indices.data_handle(), out_indices.size(),
    //     raft::Compare<Index_>(), stream));
    //   ASSERT_TRUE(raft::devArrMatch<Type_f>(
    //     out_dists_tfidf.data_handle(), out_dists.data_handle(), out_dists.size(),
...
const std::vector<SparsePreprocessInputs<float, int>> sparse_preprocess_inputs = {
  {12,                                // n_rows
   ...
   {0, 0, 1, 2, 2, 1, 1, 3, 2, 1},
   //  {0.480453, 0.7615, 0.7615, 0.960906, 1.11558, 0.960906, 0.480453, 0.480453, 0.480453,
   //  0.7615},   // tfidf {0, 3, 1, 2, 1, 2, 3, 0, 4, 7, 5, 6, 6, 5, 7, 4, 8, 4, 9, 1, 10, 1, 0,
   //  0}, // out_idx_bm25 {0, 0.281047, 0, 0, 0, 0, 0, 0.281047, 0, 0.199406, 0, 0.154671, 0,
   //  0.154671, 0, 0.199406, 0, 0.281047, 0, 0.480453, 0, 0.480453, 0, 0}, //out_dists_bm25 {0, 3,
   //  1, 2, 1, 2, 3, 0, 4, 8, 5, 10, 6, 5, 7, 8, 8, 7, 9, 1, 10, 5, 0, 0}, // out_idx_tfidf {0,
   //  0.305159, 0, 0, 0, 0, 0, 0.305159, 0, 0.167441, 0, 0.0108292, 0, 0.129295, 0, 0.0108292, 0,
   //  0.0108292, 0, 0.850086, 0, 0.0108292, 0, 0}}, // out_dists_tfidf
