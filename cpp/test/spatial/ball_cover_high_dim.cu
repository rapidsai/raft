...
#include <raft/spatial/knn/detail/ball_cover/ball_cover_high_dim.cuh>
#include <raft/spatial/knn/detail/ball_cover/utils.h>
...
template <typename value_idx, typename value_t>
class BallCoverHighDimsTest : public ::testing::TestWithParam<BallCoverInputs> {
 protected:
  void basicTest()
  {
...
INSTANTIATE_TEST_CASE_P(BallCoverHighDimsTest,
                        BallCoverHighDimsTestF,
                        ::testing::ValuesIn(ballcover_inputs));

TEST_P(BallCoverHighDimsTestF, Fit) { basicTest(); }
