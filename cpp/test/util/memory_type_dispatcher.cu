...
template <memory_type input_memory_type>
void test_mdspan_dispatched_functor()
{
  auto res          = raft::device_resources{};
  auto data         = generate_input<input_memory_type>(res);
  auto data_float   = generate_input<input_memory_type, float>(res);
  ...
  memory_type out =
    mdspan_dispatched_functor<mdbuffer<double, matrix_extent<std::uint32_t>>, functor_h>{
      functor_h{}}(res, data.view());
  EXPECT_EQ(out, functor_h::expected_output<input_memory_type>());
  /* out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_d{}}(res, data.view());
  EXPECT_EQ(out,
            functor_d::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_m{}}(res, data.view());
  EXPECT_EQ(out,
            functor_m::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_p{}}(res, data.view());
  EXPECT_EQ(out,
            functor_p::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hd{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hd::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hm{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hm::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hp::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_dm{}}(res, data.view());
  EXPECT_EQ(out,
            functor_dm::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_dp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_dp::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_mp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_mp::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hdm{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hdm::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hdp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hdp::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_dmp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_dmp::expected_output<input_memory_type>());
  out = mdspan_dispatched_functor<mdbuffer<double,
      matrix_extent<std::uint32_t>>>{functor_hdmp{}}(res, data.view());
  EXPECT_EQ(out,
            functor_hdmp::expected_output<input_memory_type>()); */
...
TEST(MdspanDispatchedFunctor, FromHost)
{
  dispatch_test::test_mdspan_dispatched_functor<memory_type::host>();
}
