                     ...
                     T svr_eps        = 0)
{
  ...
  switch (pams.loss) {
    ...
    case QN_LOSS_SVC_L1: {
      ...
      HingeLoss<T> loss(handle, D, pams.fit_intercept);
      if (sample_weight) loss.add_sample_weights(sample_weight, N, stream);
      qn_fit<T, decltype(loss)>(handle, pams, loss, X, y, Z, w0_data, f, num_iters, stream);
    } break;
    case QN_LOSS_SVC_L2: {
      ASSERT(C == 2, "qn.h: SVC-L2 loss invalid C");
      SqHingeLoss<T> loss(handle, D, pams.fit_intercept);
      if (sample_weight) loss.add_sample_weights(sample_weight, N, stream);
      qn_fit<T, decltype(loss)>(handle, pams, loss, X, y, Z, w0_data, f, num_iters, stream);
    } break;
    case QN_LOSS_SVR_L1: {
      ASSERT(C == 1, "qn.h: SVR-L1 loss invalid C");
      EpsInsHingeLoss<T> loss(handle, D, pams.fit_intercept, svr_eps);
      if (sample_weight) loss.add_sample_weights(sample_weight, N, stream);
      qn_fit<T, decltype(loss)>(handle, pams, loss, X, y, Z, w0_data, f, num_iters, stream);
    } break;
    case QN_LOSS_SVR_L2: {
      ASSERT(C == 1, "qn.h: SVR-L2 loss invalid C");
      SqEpsInsHingeLoss<T> loss(handle, D, pams.fit_intercept, svr_eps);
      if (sample_weight) loss.add_sample_weights(sample_weight, N, stream);
      qn_fit<T, decltype(loss)>(handle, pams, loss, X, y, Z, w0_data, f, num_iters, stream);
    } break;
    case QN_LOSS_ABS: {
      ...
      SimpleDenseMat<T>& z)
{
  ...
  qn_fit<T, LossFunction>(handle, pams, loss, X, y, z, w, &fx, &num_iters);
...
TEST_F(QuasiNewtonTest, binary_logistic_vs_sklearn)
{
  ...
  fx = run_api(
    handle, QN_LOSS_LOGISTIC, 2, loss_b.fit_intercept, *Xdev, *ydev, l1, l2, w0.data, z, 0, stream);
  ASSERT_TRUE(compApprox(obj_l1_b, fx));
