                                        ...
                                        const std::uint32_t normalized_norm)
{
  ...
  const auto scale = normalized_norm / sqrt(static_cast<float>(norm2));
  for (std::uint32_t i = 0; i < dim; i++) {
    datatset_ptr[tid * dim + i] *= scale;
  }
}
                      ...
                      const std::uint32_t dim)
{
  ...
  const std::uint32_t normalized_norm = std::is_same_v<T, std::uint8_t> ? 250 : 120;
