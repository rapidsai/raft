     ...
     {
       {
 ...
 
         {
           if (ps.host_dataset) {
             auto database_host = raft::make_host_matrix<DataT, int64_t>(ps.n_rows, ps.dim);
             raft::copy(database_host.data_handle(), database.data(), database.size(), stream_);
            //  raft::print_host_vector("host dataset", database_host.data_handle(), 20, std::cout);
             auto database_host_view = raft::make_host_matrix_view<const DataT, int64_t>(
               (const DataT*)database_host.data_handle(), ps.n_rows, ps.dim);
            ...
            // raft::print_device_vector("device dataset", database_view.data_handle(), 20, std::cout);
             raft::copy(
               indices_NNDescent.data(), index.graph().data_handle(), queries_size, stream_);
             if (index.distances().has_value()) {
               raft::copy(distances_NNDescent.data(),
