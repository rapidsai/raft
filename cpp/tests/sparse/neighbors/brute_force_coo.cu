...
template <typename value_idx, typename value_t>
class SparseKNNCOOTest : public ::testing::TestWithParam<SparseKNNInputs<value_idx, value_t>> {
 public:
  SparseKNNCOOTest()
 ...
 protected:
  void SetUp() override
  {
    ...
    auto rows = raft::make_device_vector<value_idx, int64_t>(handle, nnz);
    ...
    raft::sparse::convert::csr_to_coo(
      indptr.data(), int(indptr.size()), rows.data_handle(), nnz, stream);

    auto coo_struct_view = raft::make_device_coordinate_structure_view(
      rows.data_handle(), indices.data(), n_rows, params.n_cols, int(data.size()));
    auto c_matrix = raft::make_device_coo_matrix<value_t, value_idx, value_idx, value_idx>(
      handle, coo_struct_view);
    raft::update_device<value_t>(
...
typedef SparseKNNCOOTest<int, float> SparseKNNCOOTestF;
TEST_P(SparseKNNCOOTestF, Result) { compare(); }
INSTANTIATE_TEST_CASE_P(SparseKNNCOOTest, SparseKNNCOOTestF, ::testing::ValuesIn(inputs_i32_f));
